...
## Главная причина (часто в Portainer)

В списке контейнеров у `creative_collective_web` должно быть:

- **`80:80`** (не `8081:80`)
- **`443:443`** (не `8443:443`)

Let's Encrypt всегда проверяет **`http://домен:80`**. Если Caddy проброшен как `8081:80`, с интернета на порт 80 отвечает **другой** сервис → в логах **404** на `/.well-known/acme-challenge/`.

В `docker-compose.yml` в репо уже стоят `80:80` и `443:443`. В Portainer в стеке **убери** порты 8081/8443 и верни **80/443**. Если 80 занят — освободи (останови nginx/другой контейнер на 80).
